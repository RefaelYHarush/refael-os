  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-brand-black dark:text-on-brand">כספים</h2>
          <p className="text-sm text-brand-black/500 dark:text-on-brand-muted">יעדי חיסכון ומעקב התקדמות</p>
        </div>
        <button
          type="button"
          onClick={openAdd}
        ...
        <Card className="p-5 border-t-4 border-t-brand">
          <form onSubmit={handleSubmit} className="space-y-4">
            <label className="block">
              <span className="text-xs font-medium text-brand-black/500 dark:text-on-brand-muted block mb-1">שם היעד</span>
              <input
                type="text"
                value={form.name}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="block">
                <span className="text-xs font-medium text-brand-black/500 dark:text-on-brand-muted block mb-1">יעד (₪)</span>
                <input
                  type="number"
                  min="0"
              ...
              <label className="block">
                <span className="text-xs font-medium text-brand-black/500 dark:text-on-brand-muted block mb-1">נצבר עד כה (₪)</span>
                <input
                  type="number"
                  min="0"
            ...
            <label className="block">
              <span className="text-xs font-medium text-brand-black/500 dark:text-on-brand-muted block mb-1">דדליין (אופציונלי, למשל 2025-12)</span>
              <input
                type="text"
                value={form.deadline}
            ...
            return (
              <Card key={goal.id} className="p-4 border-t-4 border-t-brand-dark dark:border-t-brand">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-bold text-brand-black dark:text-on-brand">{goal.name}</h3>
                    {goal.deadline && (
                      <p className="text-xs text-brand-black/500 dark:text-on-brand-muted">יעד: {goal.deadline}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button type="button" onClick={() => openEdit(goal)} className="p-2 rounded-lg text-brand-black/400 hover:text-brand-dark dark:hover:text-brand" aria-label="ערוך">
                      <Edit2 size={16} />
                    </button>
                    <button type="button" onClick={() => deleteFinanceGoal(goal.id)} className="p-2 rounded-lg text-brand-black/400 hover:text-brand-accent-secondary" aria-label="מחק">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
                <div className="mt-3 flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-brand-black/400 dark:text-on-brand-muted">/</span>
                  <span className="text-brand-black/600 dark:text-on-brand-muted">{formatMoney(goal.targetAmount)}</span>
                  <input
                    type="number"
                    min="0"
                    value={current}
                    onChange={(e) => setCurrentAmount(goal, e.target.value)}
                    className="w-24 px-2 py-1 text-sm rounded border border-brand-black/10 dark:border-brand/20 bg-brand-white dark:bg-brand-surface text-brand-black dark:text-on-brand"
                    aria-label="עדכן סכום נצבר"
                  />
                  <span className="text-xs text-brand-black/500">עדכן נצבר</span>
                </div>
                <div className="mt-2">
